...
public:
    ...
    ~TestRewriter() { compilation.steal(std::move(alloc)); }

    void handle(const HierarchyInstantiationSyntax& decl) {
        auto &new_decl = factory.hierarchyInstantiation(decl.attributes, decl.type, decl.parameters, decl.instances, decl.semi);
        auto type = decl.type.withRawText(alloc, "abcde");
        new_decl.type = type;
        replace(decl, new_decl);
        std::cout << " new_decl: " << &new_decl << " " << "kind: " << new_decl.kind << "\n";
    }
};

class TestRewriter1 : public SyntaxRewriter<TestRewriter1> {
public:
    Compilation compilation;
    SemanticModel model;

    TestRewriter1(const std::shared_ptr<SyntaxTree>& tree) : model(compilation) {
        compilation.addSyntaxTree(tree);
    }

    void handle(const ModuleHeaderSyntax& decl) {
        printf("Module moduleKeyword:\n%s\n", decl.moduleKeyword.toString().c_str());
        //printf("Module lifetime:\n%s\n", decl.lifetime.toString().c_str());
        printf("Module name:\n%s\n", decl.name.toString().c_str());
        printf("Module imports:\n%s\n", decl.imports.toString().c_str());
        if (decl.parameters) printf("Module parameters:\n%s\n", decl.parameters->toString().c_str());
        if (decl.ports) printf("Module ports:\n%s\n", decl.ports->toString().c_str());
        printf("Module semi:\n%s\n", decl.semi.toString().c_str());
        auto portList = decl.ports;
        if (!portList)
            return;
    }
};
...
bool loadAllSources(Compilation& compilation, SourceManager& sourceManager,
    ...
    if (singleUnit) {
        ...
        for (const SourceBuffer& buffer : buffers) {
            ...
            tree = TestRewriter1(tree).transform(tree);
            printf("tree:\n%s\n", SyntaxPrinter::printFile(*tree).c_str());
            #endif
            compilation.addSyntaxTree(tree);
        }
    }
...
public:
    ...
    bool run() {
        ...
        if (succeeded) {
            OS::printE("{}", "Create Netlist\n");
            std::ifstream input("hierSetup");
            verilog::emit2(compilation.getRoot(), input);
        }
    ...
    void test() {
        for (const auto& unit : compilation.getRoot().compilationUnits) {
            std::cout << "unit: " << unit->name << "\n";
            std::cout << SyntaxPrinter().print(*unit->getSyntax()).str();
        }
    }
};
