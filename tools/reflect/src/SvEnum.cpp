...
void SvEnum::toCpp(HppFile& hppFile, std::string_view, const SvAliases&, bool) const {
    ...
    hppFile.addWithIndent(std::format("struct {} {{\n", type.name));
    hppFile.increaseIndent();

    hppFile.addWithIndent(std::format("enum Type : {} {{\n", underlyingType()));
    hppFile.increaseIndent();
    ...
    for (const auto& member : type.getCanonicalType().as<slang::ast::Scope>().members()) {
        ...
        std::string enumName = isCppReserved(enumMember.name) ? std::format("_{}", enumMember.name)
    ...
    for (auto i = 0; i < members.size(); i++) {
        hppFile.addWithIndent(std::format("{} = {}", members[i].first, members[i].second));
        if (i == members.size() - 1)
            hppFile.add("\n");
        else
            hppFile.add(",\n");
    ...
    //** SIZE **/
    hppFile.addWithIndent(
        std::format("static constexpr size_t _size = {};\n\n", type.getBitstreamWidth()));

    //** LOCAL **//
    hppFile.addWithIndent("Type type;\n");

    //** DEFAULT CONSTRUCTOR **//
    hppFile.addWithIndent(std::format("{}() = default;\n", type.name));

    //** CONSTRUCTOR **//
    hppFile.addWithIndent(std::format("{} ({} __data) {{\n", type.name, underlyingType()));
    hppFile.increaseIndent();
    hppFile.addWithIndent("switch (__data) {\n");
    hppFile.increaseIndent();
    for (const auto& member : members)
        hppFile.addWithIndent(
            std::format("case {}: type = Type::{}; break;\n", member.second, member.first));
    hppFile.addWithIndent(std::format(
    ...
    hppFile.addWithIndent(std::format("{0} (Type& type) {{ this->type = type; }}\n", type.name));
    ...
    hppFile.increaseIndent();
    for (const auto& member : members)
        hppFile.addWithIndent(std::format("case Type::{0}: os << \"{0}\"; break;\n", member.first));
    hppFile.decreaseIndent();
    hppFile.addWithIndent("}\n");
    ...
    //** OVERLOAD = OPERATOR **//
    hppFile.addWithIndent(std::format("{}& operator=(const Type t) {{\n", type.name));
    hppFile.increaseIndent();
    ...
    hppFile.addWithIndent(std::format("operator uint64_t() const {{\n"));
    hppFile.increaseIndent();
    hppFile.addWithIndent(std::format("return static_cast<uint64_t>(type);\n", type.name));
    hppFile.decreaseIndent();
    hppFile.addWithIndent("}\n\n");

    //** OVERLOAD () OPERATOR **//
    hppFile.addWithIndent(std::format("Type operator() () const {{\n"));
    hppFile.increaseIndent();
